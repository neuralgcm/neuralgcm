...
def get_surface_pressure(geopotential, geopotential_at_surface, sim_units=None):
  """Computes surface pressure by interpolating the pressure level where geopotential_at_surface - geopotential == 0.

  Args:
    geopotential: A `cx.Field` with exactly one `PressureLevels` coordinate.
    geopotential_at_surface: A `cx.Field` representing the surface geopotential.
    sim_units: An optional `SimUnits` instance. If provided, assumes input units
      are millibars for nondimensionalization.

  Returns:
    A `cx.Field` where the vertical pressure axis is reduced to a `cx.Scalar()`.
  """
  ...
  if len(levels) != 1:
    raise ValueError(
        'geopotential must have exactly one PressureLevels coordinate, '
        f'found {len(levels)}.'
    )
  [level] = levels
  ...
  def find_intercept(x, xp, fp):
    # Interpolate pressure values (fp) at geopotential heights (xp) 
    # to the target surface geopotential (x).
    return _interp(x, xp, fp)

  out_axes = {d: i for i, d in enumerate(geopotential_at_surface.coordinate.dims)}
  regrid_fn = cx.cmap(find_intercept, out_axes=out_axes)

  result = regrid_fn(
      geopotential_at_surface,
      geopotential.untag(level),
      pressure.untag(level)
  )
  
  # Replace the vertical axis in the original coordinate with Scalar()
  new_coordinate = cx.replace_axes_in_coordinate(geopotential.coordinate, level, cx.Scalar())
  return result.tag(new_coordinate)
