...
def get_surface_pressure(geopotential, geopotential_at_surface, sim_units=None):
  ...
  if not isinstance(levels, list) or len(levels) != 1:
    raise ValueError(
        f'geopotential must have exactly one PressureLevels coordinate; found {len(levels)}.'
    )
  [level] = levels
  pressure = level.fields['pressure']
  ...
  def find_intercept(data, surface_g):
    ref = surface_g - data
    return _linear_interp_with_linear_extrap(
        jnp.zeros_like(ref[0]), ref, pressure.data
    )

  out_coord = cx.replace_axes_in_coordinate(
      geopotential.coordinate, level, cx.Scalar()
  )
  out_axes = {d: i for i, d in enumerate(out_coord.dims)}
  fn = cx.cmap(find_intercept, out_axes=out_axes)
  return fn(geopotential.untag(level), geopotential_at_surface)
