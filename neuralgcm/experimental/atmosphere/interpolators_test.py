...
class GetSurfacePressureTest(parameterized.TestCase):
  ...
  def test_get_surface_pressure_1d(self):
    source_levels = coordinates.PressureLevels([100, 300, 500, 700, 900])
    geopotential_data = np.array([10.0, 20.0, 30.0, 40.0, 50.0])
    field = cx.wrap(geopotential_data, source_levels)
    surface_g = 35.0
    actual = interpolators.get_surface_pressure(field, surface_g)
    expected = cx.wrap(600.0, cx.Scalar())
    cx_testing.assert_fields_allclose(actual, expected, atol=1e-6)

  def test_get_surface_pressure_spatial(self):
    grid = coordinates.LonLatGrid.T21()
    source_levels = coordinates.PressureLevels([100, 300, 500, 700, 900])
    coords = cx.compose_coordinates(source_levels, grid)
    field_data = np.tile(
        np.array([10.0, 20.0, 30.0, 40.0, 50.0])[:, np.newaxis, np.newaxis],
        (1, *grid.shape),
    )
    field = cx.wrap(field_data, coords)
    surface_g = cx.wrap(np.full(grid.shape, 35.0), grid)

    actual = interpolators.get_surface_pressure(field, surface_g)
    expected_data = np.full(grid.shape, 600.0)
    expected_coords = cx.compose_coordinates(cx.Scalar(), grid)
    expected = cx.wrap(expected_data, expected_coords)
    cx_testing.assert_fields_allclose(actual, expected, atol=1e-6)

  def test_raises_value_error_on_invalid_levels(self):
    source_levels = coordinates.SigmaLevels.equidistant(5)
    field = cx.wrap(np.ones(5), source_levels)
    surface_g = cx.wrap(np.ones(0))
    with self.assertRaisesRegex(ValueError, 'exactly one PressureLevels'):
      interpolators.get_surface_pressure(field, surface_g)
