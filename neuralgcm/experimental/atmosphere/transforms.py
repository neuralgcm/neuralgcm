...
def _filter_fields(
  ...
  """Returns a subset of fields from `inputs` that contain `coord`."""
  return {
      k: v
      for k, v in f.items()
...
class ToModalWithDivCurl(transforms.TransformABC):
  ...
  def __call__(self, inputs: dict[str, cx.Field]) -> dict[str, cx.Field]:
    ...
    inputs = _filter_fields(inputs, grid)
    ...
    modal_outputs['divergence'] = parallelism.with_dycore_sharding(
        mesh,
        cx.cmap(dinosaur_grid.div_cos_lat)(cx.untag((u, v), ylm_grid)).tag(
            ylm_grid
        ),
    )
    modal_outputs['vorticity'] = parallelism.with_dycore_sharding(
        mesh,
        cx.cmap(dinosaur_grid.curl_cos_lat)(cx.untag((u, v), ylm_grid)).tag(
            ylm_grid
        ),
    )
...
class VelocityAndPrognosticsWithModalGradients(transforms.TransformABC):
  ...
  def _extract_features(
    ...
    if set(['vorticity', 'divergence']).issubset(inputs.keys()) and (
      ...
      cos_lat_u, cos_lat_v = cx.tag(
          cx.cmap(cos_lat_fn)(
              *cx.untag((inputs['vorticity'], inputs['divergence']), ylm_grid)
          ),
          ylm_grid,
      )
      modal_features = {}
  ...
  def __call__(self, inputs: dict[str, cx.Field]) -> dict[str, cx.Field]:
    ...
    ylm_grid = self.ylm_transform.modal_grid
    filtered_inputs = _filter_fields(inputs, ylm_grid)
