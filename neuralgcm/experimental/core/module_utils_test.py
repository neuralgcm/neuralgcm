...
class ModuleUtilsTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      dict(
          ...
          specs_list=[
              {typing.Prognostic: cx.SizedAxis('batch', 3)},
              {Ellipsis: cx.SizedAxis('ensemble', 2)},
          ],
      ),
  )
  def test_merge_vectorized_axes(self, specs_list):
    module = MockModule()
...
class StateInAxesUtilTest(parameterized.TestCase):
  ...
  def test_state_in_axes_for_coord_with_nesting(self):
    ...
    self.assertSetEqual(set(actual_outer.keys()), set(expected_outer.keys()))
    self.assertSetEqual(set(actual_inner.keys()), set(expected_inner.keys()))
