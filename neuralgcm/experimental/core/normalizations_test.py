...
class StreamingNormalizerTest(parameterized.TestCase):
  ...
  def test_stream_norm_close_to_identity_at_init(self):
    ...
    rng = np.random.RandomState(0)
    inputs = {'x': cx.wrap(rng.normal(size=(10, 2, s.size)), 'a', 'b', s)}
  ...
  def test_streaming_normalizer_first_step_estimate(self):
    ...
    inputs = {'u': cx.wrap(inputs_data, 'b', x)}
  ...
  def test_streaming_normalizer_estimates_stats_scalar(self):
    ...
    inputs = {
        'u': cx.wrap(u_data, 'batch', x),
        'v': cx.wrap(v_data, 'b'),
    }

    stat_coords = {'u': cx.Scalar(), 'v': cx.Scalar()}  # scalar stats.
    normalizer = normalizations.StreamingNormalizer(stat_coords, epsilon=0.0)
  ...
  def test_streaming_normalizer_estimates_stats_per_level(self):
    ...
    inputs = {'u': cx.wrap(data, 'b', level, xy)}
    stat_coords = {'u': level}
  ...
  def test_streaming_normalizer_estimates_stats_multiscale(self):
    ...
    stat_coords = {k: cx.Scalar() for k in means}
    normalizer = normalizations.StreamingNormalizer(stat_coords, epsilon=0.0)
    ...
    for i in range(0, n_samples, batch_size):
      inputs = {}
      for k in means:
        inputs[k] = cx.wrap(all_data[k][i : i + batch_size], 'batch', x, y)
      _ = _streaming_normalizer_apply(normalizer, inputs)
    ...
    with self.subTest('z_scales'):
      np.testing.assert_allclose(
          actual_means['z'].data, expected_means['z'], rtol=1e-4
      )
      np.testing.assert_allclose(
          actual_vars['z'].data, expected_vars['z'], rtol=1e-3
      )
  ...
  def test_streaming_normalizer_with_mask(self):
    ...
    inputs_field = cx.wrap(inputs_u, time_dim, x, y)
    inputs = {'u': inputs_field}
    stat_coords = {'u': cx.compose_coordinates(x, y)}

    # Generate random mask
    rng_jax = jax.random.key(0)
    mask_shape = (time_dim.size, x.size, y.size)
    mask_arr = jax.random.bernoulli(rng_jax, p=0.3, shape=mask_shape)
    mask = cx.wrap(mask_arr, time_dim, x, y)

    normalizer = normalizations.StreamingNormalizer(stat_coords)
    ...
    mask_np = np.array(mask_arr)  # True means masked.
    expected_mean = np.zeros((x.size, y.size))
    expected_var = np.zeros((x.size, y.size))
    for i in range(x.size):
      for j in range(y.size):
        # Select data where mask is False (valid)
