...
class UnaryFieldTower(nnx.Module):
  ...
  def __call__(self, f: cx.Field) -> cx.Field:
    def apply_net(net, x):
      return net(x)

    f = f.untag(*self.net_in_dims)
    nnx_cmap = functools.partial(cx.cmap, vmap=nnx.vmap, out_axes=f.named_axes)
    mapped_apply_net = nnx_cmap(apply_net)
    if self.apply_remat:
      mapped_apply_net = nnx.remat(mapped_apply_net)

    out = mapped_apply_net(self.neural_net, f)
