...
class Broadcast(TransformABC):
  ...
  def __call__(self, inputs: dict[str, cx.Field]) -> dict[str, cx.Field]:
    # for broadcasting field has to increase ndim, so we can attempt to
    # broadcast to the field with largest ndim.
...
class ShiftAndNormalizePerKey(TransformABC):
  ...
  `shifts` and `scales` can be a superset of input values, but if an key in the
  ...
  def __init__(
    ...
    if global_scale is not None:
      scales = jax.tree.map(lambda x: x * global_scale, scales)
...
class StreamingStatsNormalization(TransformABC):
  ...
  def __init__(
    ...
    # avoid the need to untag axes by position.
