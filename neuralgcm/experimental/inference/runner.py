...
class InferenceRunner:
  ...
  def run(self, task_id: int) -> None:
    ...
    # Assimilation Step
    dynamic_inputs_data = dynamic_inputs_forecast.get_data(
        np.timedelta64(0, 'h'), self.output_freq
    )
    dynamic_inputs = self.model.dynamic_inputs_from_xarray(dynamic_inputs_data)
